...
if(isset($_POST['add_art']))
{
    ...
    if(intval($_POST['education_id']) == 0)
    {
        $errors[] = 'Eğitim bilgisi bulunamadı.';
    }
    ...
    if(count($errors) > 0)
    {
        ...
        if($db -> insertData('education_articles', $data))
        {
...
if(isset($_POST['edit_art']))
{
    ...
    if(count($errors) > 0)
    {
        ...
        if($db -> updateData('education_articles', $data, $id))
        {
...
if(isset($_POST['del_art']))
{
    ...
    if(empty($errors))
    {
        $q = $db -> q("DELETE FROM education_articles WHERE MD5(concat('zsistem', id)) = '{$id}'");
