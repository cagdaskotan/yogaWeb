...
{
    $errors = [];
    if(empty($_POST['education_id'])) $errors[] = 'Eğitim seçimi yapmalısınız.';
    ...
    if(count($errors) > 0)
    {
        ...
        if($db -> insertData('education_subjects', $data))
        {
...
if(isset($_POST['edit_subject']))
{
    ...
    if(count($errors) > 0)
    {
        ...
        if($db -> updateData('education_subjects', $data, $id))
        {
...
if(isset($_POST['prep_subject']))
{
    ...
    $q = $db -> q("SELECT * FROM education WHERE is_delete = 0 ORDER BY id ASC");

    while($r = $db -> object($q))
    {
        if($r -> id == $education_id)
        {
    ...
    $getsubject = $db -> q("SELECT * FROM education_subjects WHERE id = {$id} ORDER BY id ASC ");
...
if(isset($_POST['get_subjects']))
{
    ...
    $q = $db -> q("SELECT * FROM education_subjects WHERE education_id = {$education_id} AND is_delete = 0 ORDER BY id ASC");
...
if(isset($_POST['add_education']))
{
    ...
    if(empty($_POST['education_title'])) $errors[] = 'Eğitim başlığı alanı boş bırakılamaz.';

    if(count($errors) > 0)
    {
        echo json_encode(['status' => 300, 'message' => $errors]);
    } else {
        $data = ['title' => $db -> rescape($_POST['education_title'])];

        if ($db -> insertData('education', $data))
        {
            echo json_encode(['status' => 200, 'message' => 'Eğitim eklendi. Yönlendiriliyorsunuz...']);
        } else {
            echo json_encode(['status' => 400, 'message' => 'İşlem sırasında bir hata oluştu.']);
        }
    }
...
if(isset($_POST['edit_education']))
{
    ...
    if(count($errors) > 0)
    {
        ...
        if($db -> updateData('education', $data, $id))
        {
            echo json_encode(['status' => 200, 'message' => 'Eğitim başlığı başarıyla güncellendi. Yönlendiriliyorsunuz...']);
        } else {
            echo json_encode(['status' => 400, 'message' => 'İşlem sırasında bir hata oluştu.']);
        }
    }
...
if(isset($_POST['delete_education']))
{
    ...
    if($db -> updateData('education', $data, $id))
    {
        echo json_encode(['status' => 200, 'message' => 'Eğitim başarıyla silindi. Yönlendiriliyorsunuz...']);
    } else {
        echo json_encode(['status' => 400, 'message' => 'İşlem sırasında bir hata oluştu.']);
    }
}
...
{
    $id = intval($_POST['id']);
    $q = $db -> q("SELECT * FROM education WHERE id = {$id} ORDER BY id ASC");
    ...
    if($db -> numrows($q) > 0)
    {
        ...
        echo json_encode(['status' => 400, 'message' => 'Eğitim başlığı bulunamadı.']);
    }
}
...
if(isset($_POST['activate']))
{
    ...
    {
        $data = ['is_active' => 0];
        $msg = 'Eğitim yayından kaldırıldı';
        $cls = 'bg-danger';
    }
